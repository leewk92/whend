...
public class PF04_TmpCallist extends Fragment {
	...
	PF04_Callist frag = PF04_Callist.newInstance();
...
/*
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
		   //inflater.inflate(R.menu.pf02__items, menu);
		    ((PF01_Main)getActivity()).setActionBarTitle(" Callist");
		}
	*/
    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
	 ...
	 //   startActivity(intent);
      ...
      //  getFragmentManager().beginTransaction().add(frag, "CTcalFrag").commit();
	    if(frag.isAdded()){
	    	Log.d("already","added");
	    }else getFragmentManager().beginTransaction().replace(R.id.realtabcontent, frag,"listtag").addToBackStack(null).commit();	//이게 돌아가야하는데..
	   // getFragmentManager().beginTransaction().add(this.getId(), frag,"listtag").addToBackStack(null).commit();	//이게 돌아가야하는데..
