...
//    JSONObject evtjson = new JSONObject();
  ...
  //달력에 표시할 일자를 배열에 넣어 구성한다. 
    ...
    private void makeCalendar()
    {
        ...
        oneday_height = ((((oneday_height >= oneday_width)?oneday_height:oneday_width) - tl.getTop()) / (maxRow+1))+13;	//revised, original : -10이 뒤에 붙어있었음.
...
//        syncDeviceCalendar();
        for(int i=1;i<=maxRow;i++ )
        {
            ...
            for(int j=1;j<=maxColumn;j++)
            {
                ...
                oneday[dayCnt].setTextDayTopPadding(-35); 		//revised Original: 없었음
    ...
    //	String[] selectionArgs = new String[]{"sampleuser@gmail.com", "com.google"}; 
    	...
    	while (cur.moveToNext()) {
        	...
        	while (cur2.moveToNext()) {
				...
				evtpropjson.put("DTSTART", cur2.getLong(1));
				evtpropjson.put("DTEND", cur2.getLong(2));
	        	...
	        	evtpropjson.put("ALL_DAY", cur2.getInt(9));
	        	evtpropjson.put("RDATE", cur2.getString(10));
	        	evtpropjson.put("AVAILABILITY", cur2.getInt(11));
    	...
    	JSONParser transmitter = new JSONParser();
    	 List<NameValuePair> params = new ArrayList<NameValuePair>();
         params.add(new BasicNameValuePair("calendar", caljson.toString()));
    	 JSONObject calReq = transmitter.makeHttpRequest("http://54.149.93.56/whend/android/","POST",params);
