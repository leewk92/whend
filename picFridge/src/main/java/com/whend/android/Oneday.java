...
public class Oneday extends View {
    ...
    List<String> textActCnts = new ArrayList<String>();
	List<Integer> textActcntTopPaddings = new ArrayList<Integer>();
	List<Paint> bgActcntPaints = new ArrayList<Paint>();
...
//        }
        for(int i=0; i<textActCnts.size(); i++){
        	 
        //	 rect.set(3, 45+i*20, width*2-3, 65+i*20);
         //    canvas.drawRoundRect(rect, 3, 3, bgActcntPaint);	//bgActcntPaints.get(i);
             canvas.drawText(textActCnts.get(i), getTextActcntLeftPadding(),getTextActcntTopPadding() + getHeight()/8*(i+1) , textActcntPaint); //height + textActcntTopPaddings.get(i)height-50 + i*this.getHeight()/8
             
            // Log.d("draw",textActCnts.get(i));
